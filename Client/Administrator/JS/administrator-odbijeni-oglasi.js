...
poljaPretrazi.forEach((poljePretrazi)=>{
    poljePretrazi.addEventListener("keypress", async function (event) {
        if(event.key === "Enter")
        {
            ...
            {
                url=`./administrator-odbijeni-oglasi.html?username=${username}&zadruga=${zadruga}&poslodavac=${poslodavac}&grad=${grad}&datumIzvrsavanja=${datumIzvrsavanja}&tip=${tip}&nacinPlacanja=${nacinPlacanja}&novacOd=${novacOd}&novacDo=${novacDo}&pretraga=${poljePretrazi.value}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
            else
            {
                url=`./administrator-odbijeni-oglasi.html?username=${username}&zadruga=${zadruga}&poslodavac=${poslodavac}&grad=${grad}&datumIzvrsavanja=${datumIzvrsavanja}&tip=${tip}&nacinPlacanja=${nacinPlacanja}&novacOd=${novacOd}&novacDo=${novacDo}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
        }
    })
...
btnsPretrazi.forEach((btnPretrazi, i)=>{
    btnPretrazi.addEventListener("click", async function () {
        if(poljaPretrazi[i].value.length!==0)
            {
                url=`./administrator-odbijeni-oglasi.html?username=${username}&zadruga=${zadruga}&poslodavac=${poslodavac}&grad=${grad}&datumIzvrsavanja=${datumIzvrsavanja}&tip=${tip}&nacinPlacanja=${nacinPlacanja}&novacOd=${novacOd}&novacDo=${novacDo}&pretraga=${poljaPretrazi[i].value}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
            else
            {
...
function selekcijaPromena(vrednost) {
    url=`./administrator-odbijeni-oglasi.html?username=${username}&zadruga=${zadruga}&poslodavac=${poslodavac}&grad=${grad}&datumIzvrsavanja=${datumIzvrsavanja}&tip=${tip}&nacinPlacanja=${nacinPlacanja}&novacOd=${novacOd}&novacDo=${novacDo}&pretraga=${pretraga}&sort=${vrednost}&stranica=${stranica}`;
    location.href=url;
}
...
let promOglasi = await fetch(`http://localhost:5258/Oglas/PreuzmiSveOdbijeneOglaseAdministratorPregledFiltriranjePretragaSortiranjeStranica/${zadruga}/${poslodavac}/${grad}/${datumIzvrsavanja}/${tip}/${nacinPlacanja}/${novacOd}/${novacDo}/${pretraga}/${sort}/${stranica}`);
...
dugmeOtkazi.addEventListener("click", function () {
    url=`./administrator-odbijeni-oglasi.html?username=${username}&pretraga=${pretraga}&sort=${sort}&stranica=${0}`;
    location.href=url;
});
...
brojeviStranica.forEach(brojStranice => {
    if(brojStranice.value != stranica)
    {
        brojStranice.addEventListener("click", function () {
            url=`./administrator-odbijeni-oglasi.html?username=${username}&zadruga=${zadruga}&poslodavac=${poslodavac}&grad=${grad}&datumIzvrsavanja=${datumIzvrsavanja}&tip=${tip}&nacinPlacanja=${nacinPlacanja}&novacOd=${novacOd}&novacDo=${novacDo}&pretraga=${pretraga}&sort=${sort}&stranica=${brojStranice.value}`;
            location.href=url;
        })
    }
    else
...
if(sledecaStranica!=null)
{
    sledecaStranica.addEventListener("click", function () {
        stranica++;
        url=`./administrator-odbijeni-oglasi.html?username=${username}&zadruga=${zadruga}&poslodavac=${poslodavac}&grad=${grad}&datumIzvrsavanja=${datumIzvrsavanja}&tip=${tip}&nacinPlacanja=${nacinPlacanja}&novacOd=${novacOd}&novacDo=${novacDo}&pretraga=${pretraga}&sort=${sort}&stranica=${stranica}`;
        location.href=url;
    })
}

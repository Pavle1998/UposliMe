...
//ucitavanje notifikacije, zbog funkcionalnosti ispod
let prosledjenOglas; 
let promOglas = await fetch(`http://localhost:5258/Notifikacija/PreuzmiNotifikacijuAdministrator/${username}/${notifikacijaId}`);
await promOglas.json().then(notif=>{
    prosledjenOglas = new Notifikacija(notif.id, notif.procitana, notif.administrator, notif.notifikacija, "oglasiDiv");
}).catch(s=>console.log("Greska kod preuzimanja notifikacije: ", s));

//notifikacija se postavlja da je procitana ukoliko nije bila
if(prosledjenOglas.procitana==false)
{
    await fetch(`http://localhost:5258/Notifikacija/PromeniNotifikacijaProcitanaAdministrator/${administrator.id}/${notifikacijaId}`,
        {
            method: 'PUT',
        });
}

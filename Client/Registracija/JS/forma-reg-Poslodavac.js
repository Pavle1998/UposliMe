...
async function dodajPoslodavcaBodi(){
    return new Promise((resolve,reject)=>{
    fetch("http://localhost:5258/Poslodavac/DodajPoslodavca/", {
            method: "POST",
            headers: {
         ...
         }).then(p=>
            {
                if(p.ok)
                {
                    ...
                    reject("Nije uspelo dodavanje poslodavca!");
                }
            })
    })
}
                                    ...
                                    if(p.status!==200){
                                        ...
                                        .then( p=> {
                                                    if(p.status!==200){ //nema takvog mejla u bazi, kreiraj 
                                                         //sacuvaj ga u bazi
                                                         dodajPoslodavcaBodi().then(p=>{otvoriHtml()})
...
zavrsiRegistracijuBtn.addEventListener("click",function(){
    if(username.value!=="" && password1.value!=="" && password2.value!=="" && pib.value!=="" && maticniBroj.value!=="" && email.value!==""  && naziv.value!=="" && fiksniTelefon.value!=="" && mobilniTelefon.value!=="" && delatnost.value!=="" && grad.options[grad.selectedIndex].value!=="Izaberi grad" && ulica.value!==""){

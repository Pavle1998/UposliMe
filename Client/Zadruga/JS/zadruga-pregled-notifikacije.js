...
let prosledjenOglas; 
let promOglas = await fetch(`http://localhost:5258/Notifikacija/PreuzmiNotifikacijuZadruga/${username}/${notifikacijaId}`);
await promOglas.json().then(notif=>{
    prosledjenOglas = new Notifikacija(notif.id, notif.procitana, notif.zadruga, notif.notifikacija, "oglasiDiv");
...
{
    await fetch(`http://localhost:5258/Notifikacija/PromeniNotifikacijaProcitanaZadruga/${zadruga.id}/${notifikacijaId}`,
        {
            method: 'PUT',
        });
}

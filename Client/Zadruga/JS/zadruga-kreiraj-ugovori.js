...
poljaPretrazi.forEach((poljePretrazi)=>{
    poljePretrazi.addEventListener("keypress", async function (event) {
        if(event.key === "Enter")
        {
            ...
            {
                url=`./zadruga-kreiraj-ugovori.html?username=${username}&pretraga=${poljePretrazi.value}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
            else
            {
                url=`./zadruga-kreiraj-ugovori.html?username=${username}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
        }
    })
...
btnsPretrazi.forEach((btnPretrazi, i)=>{
    btnPretrazi.addEventListener("click", async function () {
        if(poljaPretrazi[i].value.length!==0)
            {
                url=`./zadruga-kreiraj-ugovori.html?username=${username}&pretraga=${poljaPretrazi[i].value}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
            else
            {
...
function selekcijaPromena(vrednost) {
    url=`./zadruga-kreiraj-ugovori.html?username=${username}&pretraga=${pretraga}&sort=${vrednost}&stranica=${stranica}`;
    location.href=url;
}
...
let oglasi = [];
let brojOglasa;
let promOglasi = await fetch(`http://localhost:5258/Oglas/PreuzmiSveOglaseZadrugaPravljenjeUgovoraPretragaSortiranjeStranica/${username}/${pretraga}/${sort}/${stranica}`);
...
function prikaziOglase(niz) {
    ...
    if(niz.length!=0)
    {
        ...
        let dugmeOtvoriOglas = document.querySelectorAll(".dugmeAplicirajVrednost");
        dugmeOtvoriOglas.forEach(dugme => {
            dugme.classList.replace("dugmeAplicirajVrednost","otvoriOglas");
            dugme.value="Otvori oglas";
        })
...
function omoguciDugmeOtvoriOglas() {
    let dugmeOtvoriOglas = document.querySelectorAll(".otvoriOglas");
    dugmeOtvoriOglas.forEach(dugme => {
        dugme.addEventListener("click", function () {
            location.href="../korisnikZadruga/zadruga-pregled-oglasa-kreiranje-ugovora.html?username="+username+"&oglas="+dugme.id;
        })
    })
}
...
function omoguciNaslovKlik() {
    ...
    btnNaslov.forEach(btn=>{
        btn.addEventListener("click", function () {
            location.href="../korisnikZadruga/zadruga-pregled-oglasa-kreiranje-ugovora.html?username="+username+"&oglas="+btn.id;
        })
    })
}
...
brojeviStranica.forEach(brojStranice => {
    if(brojStranice.value != stranica)
    {
        brojStranice.addEventListener("click", function () {
            url=`./zadruga-kreiraj-ugovori.html?username=${username}&pretraga=${pretraga}&sort=${sort}&stranica=${brojStranice.value}`;
            location.href=url;
        })
    }
    else
...
if(sledecaStranica!=null)
{
    sledecaStranica.addEventListener("click", function () {
        stranica++;
        url=`./zadruga-kreiraj-ugovori.html?username=${username}&pretraga=${pretraga}&sort=${sort}&stranica=${stranica}`;
        location.href=url;
    })
}

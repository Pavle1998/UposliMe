...
//div za desne labele
    ...
    otkaziBtn.className="Dugme otkaziBtn";
    otkaziBtn.innerHTML="Otkazi kreiranje ugovora";
    divDugme.appendChild(otkaziBtn);

    
    let kreirajBtn = document.createElement("button");
    kreirajBtn.className="Dugme kreirajBtn";
    kreirajBtn.innerHTML="Kreiraj ugovor";
    divDugme.appendChild(kreirajBtn);
...
//funkcija za dugme otkazi
otkaziBtn.addEventListener("click",function(){
    history.back();
})

//funkcija za dugme obrisi
kreirajBtn.addEventListener("click", function(){
    prosledjenOglas.oglasiZadrugari.forEach(z => {
        fetch(`http://localhost:5258/Ugovor/DodajUgovor/${z.zadrugar.id}/${zadruga.id}/${prosledjenOglas.poslodavac.id}/${prosledjenOglas.id}`,          
            {
                method: "POST"
            }).then(p=>
                {
    ...
    prosledjenOglas.oglasiZadrugari.forEach(z => {
        let poruka = `Napravljen je ugovor za posao pod nazivom "${prosledjenOglas.naziv}" izmedju zadruge "${zadruga.naziv}", poslodavca "${prosledjenOglas.poslodavac.naziv}" i zadrugara "${z.zadrugar.ime} ${z.zadrugar.prezime}".`;
        fetch(`http://localhost:5258/Notifikacija/DodajNotifikacijuZaPoslodavca/${poruka}/${prosledjenOglas.poslodavac.id}`, 
        {
            method: "POST"
        });
        fetch(`http://localhost:5258/Notifikacija/DodajNotifikacijuZaZadrugara/${poruka}/${z.zadrugar.id}`, 
        {
            method: "POST"
        });
    });
    fetch(`http://localhost:5258/Oglas/KreiraniUgovori/${prosledjenOglas.id}`,          
    {
        method: "PUT"
    }).then(p=>
        {
            location=`../../Zadruga/korisnikZadruga/zadruga-kreiraj-ugovori.html?username=${username}`;
        });
...
poslodavacBtn.addEventListener("click",function(){  
    location.href="../../Zadruga/korisnikZadruga/zadruga-prikaz-korisnika.html?username="+username+"&poslodavac="+poslodavacBtn.id;
})
...
zadrugarBtns.forEach(zadrugarBtn=>{
    zadrugarBtn.addEventListener("click",function(){  
        location.href="../../Zadruga/korisnikZadruga/zadruga-prikaz-korisnika.html?username="+username+"&zadrugar="+zadrugarBtn.id;
    })
})
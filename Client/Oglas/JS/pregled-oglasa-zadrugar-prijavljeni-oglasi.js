...
obrisiBtn.addEventListener("click", function () {
    fetch(`http://localhost:5258/Zadrugar/OtkaziAplikaciju/${prosledjenOglas.id}/${zadrugar.id}`,
            {
                method: "DELETE"
            }).then(p=>
                {
                    location.href=`./prijavljeni-oglasi.html?username=${username}`;
                });
})

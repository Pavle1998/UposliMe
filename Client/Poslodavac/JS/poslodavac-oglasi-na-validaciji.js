...
poljaPretrazi.forEach((poljePretrazi)=>{
    poljePretrazi.addEventListener("keypress", async function (event) {
        if(event.key === "Enter")
        {
            ...
            {
                url=`./poslodavac-oglasi-na-validaciji.html?username=${username}&pretraga=${poljePretrazi.value}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
            else
            {
                url=`./poslodavac-oglasi-na-validaciji.html?username=${username}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
        }
    })
...
btnsPretrazi.forEach((btnPretrazi, i)=>{
    btnPretrazi.addEventListener("click", async function () {
        if(poljaPretrazi[i].value.length!==0)
            {
                url=`./poslodavac-oglasi-na-validaciji.html?username=${username}&pretraga=${poljaPretrazi[i].value}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
            else
            {
...
function selekcijaPromena(vrednost) {
    url=`./poslodavac-oglasi-na-validaciji.html?username=${username}&pretraga=${pretraga}&sort=${vrednost}&stranica=${stranica}`;
    location.href=url;
}
...
let oglasi = [];
let brojOglasa;
let promOglasi = await fetch(`http://localhost:5258/Oglas/PreuzmiSveOglasePoslodavacNaValidacijiPretragaSortiranjeStranica/${username}/${pretraga}/${sort}/${stranica}`);
...
function prikaziOglase(niz) {
    ...
    if(niz.length!=0)
    {
        ...
        let dugmeOtkaziValidaciju = document.querySelectorAll(".dugmeAplicirajVrednost");
        dugmeOtkaziValidaciju.forEach(dugme => {
            dugme.classList.replace("dugmeAplicirajVrednost","dugmeOtkaziValidaciju");
            dugme.value="Otkazi validaciju";
        })
        omoguciDugmeIzmeni();
        omoguciDugmeObrisi();
        omoguciNaslovKlik();
...
function omoguciDugmeObrisi() {
    ...
    let dugmeOtkaziValidaciju = document.querySelectorAll(".dugmeOtkaziValidaciju");
    dugmeOtkaziValidaciju.forEach(btn=>{
...
function omoguciNaslovKlik() {
    ...
    btnNaslov.forEach(btn=>{
        btn.addEventListener("click", function () {
            location.href="../korisnikPoslodavac/poslodavac-pregled-oglasa-validacija.html?username="+username+"&oglas="+btn.id;
        })
    })
}
...
brojeviStranica.forEach(brojStranice => {
    if(brojStranice.value != stranica)
    {
        brojStranice.addEventListener("click", function () {
            url=`./poslodavac-oglasi-na-validaciji.html?username=${username}&pretraga=${pretraga}&sort=${sort}&stranica=${brojStranice.value}`;
            location.href=url;
        })
    }
    else
...
if(sledecaStranica!=null)
{
    sledecaStranica.addEventListener("click", function () {
        stranica++;
        url=`./poslodavac-oglasi-na-validaciji.html?username=${username}&pretraga=${pretraga}&sort=${sort}&stranica=${stranica}`;
        location.href=url;
    })
}

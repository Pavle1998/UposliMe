...
poljaPretrazi.forEach((poljePretrazi)=>{
    poljePretrazi.addEventListener("keypress", async function (event) {
        if(event.key === "Enter")
        {
            ...
            {
                url=`./poslodavac-odaberi-radnike.html?username=${username}&pretraga=${poljePretrazi.value}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
            else
            {
                url=`./poslodavac-odaberi-radnike.html?username=${username}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
        }
    })
...
btnsPretrazi.forEach((btnPretrazi, i)=>{
    btnPretrazi.addEventListener("click", async function () {
        if(poljaPretrazi[i].value.length!==0)
            {
                url=`./poslodavac-odaberi-radnike.html?username=${username}&pretraga=${poljaPretrazi[i].value}&sort=${sort}&stranica=${0}`;
                location.href=url;
            }
            else
            {
...
function selekcijaPromena(vrednost) {
    url=`./poslodavac-odaberi-radnike.html?username=${username}&pretraga=${pretraga}&sort=${vrednost}&stranica=${stranica}`;
    location.href=url;
}
...
let oglasi = [];
let brojOglasa;
let promOglasi = await fetch(`http://localhost:5258/Oglas/PreuzmiSveOglasePoslodavacOdabirRadnikaPretragaSortiranjeStranica/${username}/${pretraga}/${sort}/${stranica}`);
...
function prikaziOglase(niz) {
    ...
    if(niz.length!=0)
    {
        ...
        //zameni klasu dugmeAplicirajVrednost u dugmeOdaberiRadnike
        let dugmeOdaberiRadnike = document.querySelectorAll(".dugmeAplicirajVrednost");
        dugmeOdaberiRadnike.forEach(dugme => {
            dugme.classList.replace("dugmeAplicirajVrednost","dugmeOdaberiRadnike");
            dugme.value="Odaberi radnike";
        })
        /*let naslovVrednost = document.querySelectorAll(".naslovVrednost");
        naslovVrednost.forEach(naslov => {
            naslov.style.cursor = 'default'; 
        })*/

        omoguciDugmeOdaberiRadnike();
        omoguciNaslovKlik();
    }
    else
    {
...
function omoguciDugmeOdaberiRadnike() {
    
    let dugmeOdaberiRadnike = document.querySelectorAll(".dugmeOdaberiRadnike");
    dugmeOdaberiRadnike.forEach(btn=>{
        btn.addEventListener("click", function () {
           location.href=`./poslodavac-odabir-radnika.html?username=${username}&oglas=${btn.id}`;
        })
    })
}
...
brojeviStranica.forEach(brojStranice => {
    if(brojStranice.value != stranica)
    {
        brojStranice.addEventListener("click", function () {
            url=`./poslodavac-odaberi-radnike.html?username=${username}&pretraga=${pretraga}&sort=${sort}&stranica=${brojStranice.value}`;
            location.href=url;
        })
    }
    else
...
if(sledecaStranica!=null)
{
    sledecaStranica.addEventListener("click", function () {
        stranica++;
        url=`./poslodavac-odaberi-radnike.html?username=${username}&pretraga=${pretraga}&sort=${sort}&stranica=${stranica}`;
        location.href=url;
    })
}

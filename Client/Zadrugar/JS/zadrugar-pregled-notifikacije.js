...
let prosledjenOglas; 
let promOglas = await fetch(`http://localhost:5258/Notifikacija/PreuzmiNotifikacijuZadrugar/${username}/${notifikacijaId}`);
await promOglas.json().then(notif=>{
    prosledjenOglas = new Notifikacija(notif.id, notif.procitana, notif.zadrugar, notif.notifikacija, "oglasiDiv");
...
{
    await fetch(`http://localhost:5258/Notifikacija/PromeniNotifikacijaProcitanaZadrugar/${zadrugar.id}/${notifikacijaId}`,
        {
            method: 'PUT',
        });
}

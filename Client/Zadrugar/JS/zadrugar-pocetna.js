...
let promOglasi = await fetch(`http://localhost:5258/Oglas/PreuzmiSveOglaseZadrugarPregledFiltriranjePretragaSortiranjeStranica/${username}/${grad}/${datumIzvrsavanja}/${tip}/${nacinPlacanja}/${novacOd}/${novacDo}/${pretraga}/${sort}/${stranica}`);
...
//dugme "Apliciraj" funkcionalnost
function omoguciDugmeApliciraj() {
    let btnsApliciraj = document.querySelectorAll(".dugmeAplicirajVrednost");
    btnsApliciraj.forEach(btn=>{
        btn.addEventListener("click", function () {
            fetch(`http://localhost:5258/Zadrugar/Apliciranje/${btn.id}/${zadrugar.id}`,
            {
                method: "POST"
            }).then(p=>
                {
...
function omoguciNaslovKlik() {
    ...
    btnNaslov.forEach(btn=>{
        btn.addEventListener("click", function () {
            location.href="../korisnikZadrugar/zadrugar-pregled-oglasa-pocetna.html?username="+username+"&oglas="+btn.id;
        })
    })
}
